		...
		$.getJSON(url, function(data) {
			...
			for (let step = 2; step < jsonLength; step++) {
				...
				if (data.values[step][3]) {
					...
					if (data.values[step][0] === 'Deposition' ||
						data.values[step][0] === 'Wet processing' ||
					data.values[step][0] === 'Etching') {
						
						
					
						if (checkToolDOM == null) {
							$('#tableDiv_left #tableBody_left').append('<tr id="'+data.values[step][0]+'" class="table-secondary td-center '+sectionClass+'"><th scope"row" colspan="4">'+data.values[step][0]+'</th></tr>');
						}
						...
						if (checkToolDOM == null) {
							$('#tableDiv_right #tableBody_right').append('<tr id="'+data.values[step][0]+'" class="table-secondary td-center '+sectionClass+'"><th scope"row" colspan="4">'+data.values[step][0]+'</th></tr>');
						}
